...
def get_snowball(symbol):
    ...
    kline = ball.kline(symbol, days=365 * 6).get('data')
    item, column = kline.get('item'), kline.get('column')
    ...
    df['近6月均线'] = df['收盘Close'].rolling(int(year_days / 2)).mean()
    ...
    df['近5日均线收益率'] = computed_annualized(int(year_days / 2), df['近5日均线'], df['收盘Close'])
...
def simulate_render(**args):
    ...
    df = get_snowball(symbol)
    hs300df = get_snowball('SH000300')
...
class Account:
    ...
    def render(self):
        ...
        ax1.plot_date(date_index, df['近5日均线'], '--', label="近5日均线", color="red")
        ...
        ax2.plot_date(hs300df['日期Date'], hs300df['近6月收益率'], '--', label='000300', color="orange")
        ax2.plot_date(date_index, df['近6月收益率'], '-', label="近6月收益率", color="skyblue")
        ax2.plot_date(date_index, df['近5日均线收益率'], '--', label="近5日均线收益率", color="skyblue")
    ...
    def computed_annual_next4(self):
        ...
        annual_sell = round(self.annual_sell + hs300_factor, 3)
        annual_buy = round(self.annual_buy + hs300_factor, 3)
...
if __name__ == '__main__':
    ...
    simulate_render(deal_type=2, annual_sell=20, annual_buy=0)  # 年化 25.95/14.28 持仓时长68
    # simulate_render(deal_type=3, annual_sell=18, annual_buy=7)  # 年化 28.01/14.34 持仓时长70.7
    # simulate_render(deal_type=4, annual_sell=19, annual_buy=8, hs300_ratio=-0.05)  # 年化 32.02/16.02 持仓时长56.24
    # simulate_render(deal_type=4, annual_sell=20, annual_buy=0, hs300_ratio=0)  # 年化 32.02/16.02 持仓时长56.24
